...
export function RegisterForm({ invitationToken, invitationEmail, companyName }: { invitationToken?: string; invitationEmail?: string; companyName?: string }) {
  ...
  return <form action={action} className="grid gap-4">
    ...
    <label><span className="label mb-1 block">Correo</span><input className="field" name="email" type="email" autoComplete="email" required readOnly={Boolean(invitationEmail)} defaultValue={invitationEmail ?? state.fields?.email} /></label>
    {invitationToken ? <div className="rounded-lg bg-subtle p-3 text-sm"><strong>Empresa</strong><p>{companyName}</p></div> : <label><span className="label mb-1 block">Nombre de la empresa</span><input className="field" name="companyName" autoComplete="organization" required defaultValue={state.fields?.companyName} /></label>}
