...
export function LoginForm({ publicRegistrationEnabled }: { publicRegistrationEnabled: boolean }) {
  ...
  return <form action={action} className="grid gap-4">
    ...
    <p className="text-center text-sm text-slate-600">{publicRegistrationEnabled ? <>¿Aún no tienes cuenta? <Link href="/registro" className="font-bold text-obra-yellowDark hover:underline">Crear cuenta</Link></> : <>Acceso limitado a la beta privada. <Link href="/contacto" className="font-bold text-obra-yellowDark hover:underline">Solicitar acceso</Link></>}</p>
  </form>;
}
