...
export function Client360ActivityOverview({
  ...
  return <section className={`grid min-w-0 gap-3 ${className}`} aria-labelledby={`${id}-activity-title`}>
    ...
    <section className="rounded-xl border border-border bg-surface p-2" aria-label="Filtros de actividad">
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(12rem,1.35fr)_minmax(8rem,.8fr)_minmax(8rem,.8fr)_minmax(8rem,.8fr)_auto_auto_auto]">
        ...
        {filtersActive ? <button type="button" onClick={clearFilters} className="min-h-10 px-2 text-[10px] font-bold text-brand-strong hover:underline">Limpiar</button> : null}
      </div>
    </section>
...
function ActivityRow({ event, isLast }: { event: ClientActivityEvent; isLast: boolean }) {
  ...
  return <li className="grid min-w-0 grid-cols-[4.2rem_1rem_2.25rem_minmax(0,1fr)] items-start gap-2 px-3 py-3 lg:grid-cols-[5rem_1rem_2rem_minmax(0,1fr)_auto_auto_auto] lg:items-center lg:gap-2 lg:py-1.5">
    ...
    <span className={`inline-flex h-9 w-9 items-center justify-center rounded-xl lg:h-7 lg:w-7 lg:rounded-lg ${eventIconTones[event.kind]}`}><Icon size={14} aria-hidden="true" /></span>
