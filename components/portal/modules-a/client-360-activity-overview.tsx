...
export function Client360ActivityOverview({
  ...
  return <section className={`grid min-w-0 gap-2.5 ${className}`} aria-labelledby={`${id}-activity-title`}>
    ...
    <section className="rounded-xl border border-border bg-surface p-2 lg:p-0.5" aria-label="Filtros de actividad">
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(12rem,1.35fr)_minmax(8rem,.8fr)_minmax(8rem,.8fr)_minmax(8rem,.8fr)_auto_auto_auto]">
        ...
        <details className="relative"><summary className="flex min-h-10 cursor-pointer list-none items-center justify-center gap-2 rounded-lg border border-border px-3 text-[10px] font-bold text-content lg:min-h-8 lg:px-2.5"><CalendarDays size={14} aria-hidden="true" /> Fechas</summary><div className="absolute left-0 z-30 mt-2 grid w-64 max-w-[calc(100vw-3rem)] gap-2 rounded-xl border border-border bg-surface p-3 shadow-xl sm:left-auto sm:right-0"><label htmlFor={`${id}-from`} className="grid gap-1 text-[9px] font-semibold text-content-secondary"><span>Desde</span><input id={`${id}-from`} type="date" value={filters.dateFrom} max={filters.dateTo || undefined} onChange={(event) => updateFilter("dateFrom", event.target.value)} className="min-h-10 rounded-lg border border-border bg-surface px-3 text-[10px] text-content" /></label><label htmlFor={`${id}-to`} className="grid gap-1 text-[9px] font-semibold text-content-secondary"><span>Hasta</span><input id={`${id}-to`} type="date" value={filters.dateTo} min={filters.dateFrom || undefined} onChange={(event) => updateFilter("dateTo", event.target.value)} className="min-h-10 rounded-lg border border-border bg-surface px-3 text-[10px] text-content" /></label></div></details>
        {safeMoreFiltersHref ? <Link href={safeMoreFiltersHref} className="secondary-button justify-center lg:min-h-8 lg:px-2.5 lg:py-0"><SlidersHorizontal size={14} aria-hidden="true" /> Más filtros</Link> : null}
        {safeExportHref ? <Link href={safeExportHref} className="secondary-button justify-center lg:min-h-8 lg:px-2.5 lg:py-0"><Download size={14} aria-hidden="true" /> Exportar</Link> : null}
        {filtersActive ? <button type="button" onClick={clearFilters} className="min-h-10 px-2 text-[10px] font-bold text-brand-strong hover:underline lg:min-h-8">Limpiar</button> : null}
      </div>
    </section>
...
function ActivityMetricCard({ metric }: { metric: ClientActivityMetric }) {
  ...
  return <article className="min-w-0 rounded-xl border border-border bg-surface p-2"><div className="flex items-center gap-2.5"><span className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl lg:h-8 lg:w-8 lg:rounded-lg ${toneClasses[tone]}`}><Icon size={16} aria-hidden="true" /></span><span className="min-w-0"><small className="block truncate text-[9px] font-semibold text-content-secondary">{presentation.label}</small><strong className="mt-0.5 block text-lg font-black tabular-nums text-content">{metric.value == null ? "—" : metric.value.toLocaleString("es-ES")}</strong>{metric.detail ? <span className="mt-0.5 block truncate text-[8px] font-semibold text-content-secondary">{metric.detail}</span> : null}</span></div></article>;
}
...
function ActivityRow({ event, isLast }: { event: ClientActivityEvent; isLast: boolean }) {
  ...
  return <li className="grid min-w-0 grid-cols-[4.2rem_1rem_2.25rem_minmax(0,1fr)] items-start gap-2 px-3 py-3 md:grid-cols-[5rem_1rem_1.75rem_minmax(0,1fr)_auto_auto_auto] md:items-center md:gap-2 md:py-0.5">
    <time dateTime={event.occurredAt} className="pt-0.5 text-[8px] leading-4 text-content-secondary md:pt-0"><strong className="block font-semibold text-content-secondary">{dateFormatter.format(date)}</strong><span>{timeFormatter.format(date)}</span></time>
    <span className="relative flex h-full min-h-9 justify-center pt-3 md:min-h-8 md:items-center md:pt-0"><span className="h-2 w-2 rounded-full bg-brand ring-2 ring-brand-soft" aria-hidden="true" />{!isLast ? <span className="absolute bottom-[-1.05rem] top-5 border-l border-brand/30 md:bottom-[-.8rem] md:top-1/2" aria-hidden="true" /> : null}</span>
    <span className={`inline-flex h-9 w-9 items-center justify-center rounded-xl md:h-6 md:w-6 md:rounded-lg ${eventIconTones[event.kind]}`}><Icon size={13} aria-hidden="true" /></span>
    <span className="min-w-0"><span className="flex flex-wrap items-center gap-2"><strong className="text-[10px] text-content">{event.title}</strong>{event.status ? <span className={`inline-flex rounded-full px-2 py-0.5 text-[8px] font-bold md:hidden ${toneClasses[statusTone]}`}>{event.status.label}</span> : null}</span>{event.description ? <p className="mt-1 text-[9px] leading-4 text-content-secondary md:truncate">{event.description}</p> : null}<span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[8px] text-content-tertiary md:hidden">{event.scope.entityLabel ? <span>{event.scope.entityLabel}</span> : null}{event.source ? <span>Origen: {event.source.label}</span> : null}{event.actor ? <span>{event.actor.name}</span> : null}</span></span>
    <span className="hidden max-w-36 truncate text-[8px] font-semibold text-content-secondary md:block">{event.scope.entityLabel ?? ""}</span>
    <span className="hidden items-center gap-2 md:flex">{event.actor ? <><span className="inline-flex h-6 w-6 shrink-0 items-center justify-center overflow-hidden rounded-full bg-subtle text-content-secondary">{avatar ? <Image src={avatar} alt="" width={24} height={24} unoptimized className="h-full w-full object-cover" /> : <UserRound size={12} aria-hidden="true" />}</span><span className="max-w-28 truncate text-[8px] font-semibold text-content-secondary">{event.actor.name}</span></> : null}</span>
    <span className="hidden items-center justify-end gap-2 md:flex">{event.status ? <span className={`inline-flex rounded-full px-2 py-1 text-[8px] font-bold ${toneClasses[statusTone]}`}>{event.status.label}</span> : null}{href ? <Link href={href} aria-label={`Abrir entidad de ${event.title}`} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-content-secondary hover:bg-subtle hover:text-content"><MoreHorizontal size={15} aria-hidden="true" /></Link> : null}</span>
    {href ? <Link href={href} className="col-start-4 mt-1 inline-flex min-h-8 items-center text-[9px] font-bold text-brand-strong hover:underline md:hidden">Ver entidad</Link> : null}
  </li>;
}
