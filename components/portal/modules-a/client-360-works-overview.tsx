...
function WorkDesktopRows({ work, expanded, onExpand, money }: { work: ClientWorkRecord; expanded: boolean; onExpand: () => void; money: Intl.NumberFormat }) {
  return <><tr className={expanded ? "bg-brand-soft/40" : "hover:bg-subtle/70"}><td className="max-w-[21rem] px-3 py-3"><WorkIdentity work={work} /></td><td className="px-3 py-3">{work.status ? <StatusBadge label={work.status} tone={work.statusTone} /> : <MissingValue />}</td><td className="w-32 px-3 py-3"><ProgressValue value={work.progressPercent} /></td><td className="px-3 py-3 text-right"><strong className="block tabular-nums text-content">{formatMoney(work.contractedAmount, money)}</strong>{work.contractedLabel ? <span className="mt-1 block text-[8px] text-content-secondary">{work.contractedLabel}</span> : null}</td><td className="px-3 py-3 text-right"><strong className={`block tabular-nums ${toneText(work.marginTone)}`}>{formatPercent(work.estimatedMarginPercent)}</strong>{finite(work.estimatedMarginAmount) ? <span className="mt-1 block text-[8px] tabular-nums text-content-secondary">{formatMoney(work.estimatedMarginAmount, money)}</span> : null}</td><td className="px-3 py-3"><span className="block text-content">{formatDate(work.endAt)}</span>{work.endDetail ? <span className="mt-1 block text-[8px] text-content-secondary">{work.endDetail}</span> : null}</td><td className="px-3 py-3"><Responsible work={work} /></td><td className="px-3 py-3"><WorkActions work={work} expanded={expanded} onExpand={onExpand} /></td></tr>{expanded && work.details ? <tr><td colSpan={8} className="border-t border-brand/20 bg-subtle/50 p-0"><WorkExpandedDetails details={work.details} /></td></tr> : null}</>;
}
