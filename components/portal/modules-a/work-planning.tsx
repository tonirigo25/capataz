...
function taskTone(task: PlanningTaskView, now = new Date()) {
  if (taskIsLate(task, now)) return "late";
...
function durationDays(task: PlanningTaskView) {
  ...
  if (start && end) return Math.max(1, Math.ceil((end.getTime() - start.getTime()) / DAY) + 1);
  if (task.estimatedMinutes) return Math.max(1, Math.ceil(task.estimatedMinutes / 480));
  return null;
}
...
function PlanningTimeline({ work, tasks, compact = false, query = "", status = "all", zoom = 1 }: { work: PlanningWorkView; tasks: PlanningTaskView[]; compact?: boolean; query?: string; status?: string; zoom?: number }) {
  const range = planningRange(tasks, work);
  const filtered = tasks.filter((task) => task.title.toLowerCase().includes(query.toLowerCase()) && (status === "all" || task.status === status));
  const hierarchy = buildHierarchy(filtered).slice(0, compact ? 15 : 30);
  ...
  return (
    <div className="planning-gantt-shell" style={{ "--planning-row": `${rowHeight}px` } as React.CSSProperties}>
      <div className="planning-gantt-table" aria-label="Tareas planificadas">
        <div className="planning-gantt-heading"><span>Actividad</span><span>{compact ? "Duración" : "Responsable"}</span><span>Inicio</span><span>Fin</span></div>
        {hierarchy.length ? hierarchy.map(({ task, depth, hasChildren }, index) => <Link key={task.id} href={`/tareas/${task.id}`} className="planning-gantt-task-row" style={{ "--task-depth": depth } as React.CSSProperties}>
          <span className="planning-gantt-task-title"><span className={`planning-task-dot planning-task-dot--${taskTone(task, new Date(work.nowIso))}`} />{hasChildren ? <GitBranch size={12} aria-hidden="true" /> : <span className="planning-leaf-index">{String(index + 1).padStart(2, "0")}</span>}<strong>{task.title}</strong></span>
          <span>{compact ? `${durationDays(task) ?? "—"} días` : task.assigneeName ?? "Sin asignar"}</span><span>{formatCompactDate(task.startsAt)}</span><span>{formatCompactDate(task.dueAt)}</span>
      ...
      <div className="planning-gantt-canvas" style={{ width: timelineWidth, minHeight: 34 + hierarchy.length * rowHeight }}>
          ...
          <div className="planning-today-line" style={{ left: Math.max(0, Math.min(timelineWidth, ((new Date(work.nowIso).getTime() - range.start.getTime()) / DAY) * dayWidth)) }}><span>Hoy</span></div>
          {hierarchy.map(({ task }, index) => {
            ...
            if (!start || !end || end < start) return <span key={task.id} className="planning-gantt-unscheduled" style={{ top: 34 + index * rowHeight }}>Sin fechas válidas</span>;
            return <Link key={task.id} href={`/tareas/${task.id}`} tabIndex={-1} aria-hidden="true" className={`planning-gantt-bar planning-gantt-bar--${taskTone(task, new Date(work.nowIso))}`} style={{ top: 34 + index * rowHeight, left, width }} title={`${task.title} · ${formatCompactDate(task.startsAt)} — ${formatCompactDate(task.dueAt)}`}><span style={{ width: `${progress ?? 0}%` }} /><strong>{task.title}</strong></Link>;
          })}
        </div>
      </div>
    </div>
...
export function WorkPlanningSummary({ work, tasks, canManage }: PlanningProps) {
  ...
  const [summaryZoom, setSummaryZoom] = useState(.8);
  const completed = tasks.filter((task) => task.status === "completed");
  const late = tasks.filter((task) => taskIsLate(task, new Date(work.nowIso)));
  const notStarted = tasks.filter((task) => ["inbox", "planned"].includes(task.status));
  ...
  const upcoming = tasks.filter((task) => /hito/i.test(task.title) && task.dueAt && !terminalStatuses.has(task.status)).sort((a, b) => new Date(a.dueAt!).getTime() - new Date(b.dueAt!).getTime()).slice(0, 5);
  const people = Array.from(new Set(tasks.map((task) => task.assigneeName).filter(Boolean))) as string[];
  const visibleMaterials = work.materials.slice(0, 4);
  const inspections = work.events.filter((event) => /inspecci|visita|control/i.test(event.title)).slice(0, 4);
  return <div className="planning-summary-grid">
    <section className="planning-card planning-summary-timeline"><header><div><p className="planning-eyebrow">Cronograma de la obra</p><h2>Planificación registrada</h2></div><Link href={`/obras/${work.id}/planificacion/gantt`}>Abrir Gantt completo</Link></header><div className="planning-summary-controls"><select aria-label="Filtrar cronograma por estado" value={summaryStatus} onChange={(event) => setSummaryStatus(event.target.value)}><option value="all">Todas las tareas</option><option value="inbox">Sin iniciar</option><option value="planned">Planificadas</option><option value="in_progress">En curso</option><option value="completed">Completadas</option></select><div className="planning-legend"><span data-tone="planned">Planificada</span><span data-tone="done">Verificada</span><span data-tone="late">Retraso</span></div><div className="planning-summary-zoom"><button type="button" aria-label="Alejar cronograma" onClick={() => setSummaryZoom((value) => Math.max(.6, value - .1))}><Minus size={13} /></button><span>Semana</span><button type="button" aria-label="Acercar cronograma" onClick={() => setSummaryZoom((value) => Math.min(1.4, value + .1))}><Plus size={13} /></button></div></div><PlanningTimeline work={work} tasks={tasks} compact status={summaryStatus} zoom={summaryZoom} /></section>
    <section className="planning-card planning-summary-metrics"><header><h2>Resumen de planificación</h2></header><div className="planning-metric-grid"><PlanningMetric label="Promedio de señales verificadas" value={verifiedProgress == null ? "—" : `${verifiedProgress}%`} detail={verifiedProgress == null ? "Sin checklist suficiente" : `${progressValues.length} tareas con señal; no es avance ponderado`} tone="success" /><PlanningMetric label="Actividades totales" value={String(tasks.length)} detail="Tareas vinculadas" /><PlanningMetric label="Completadas" value={String(completed.length)} detail="Estado confirmado" tone="success" /><PlanningMetric label="Con retraso" value={String(late.length)} detail="Vencimiento superado" tone={late.length ? "danger" : "success"} /><PlanningMetric label="Sin empezar" value={String(notStarted.length)} detail="Inbox o planificadas" /><PlanningMetric label="Fin estimado" value={formatCompactDate(work.dueAt)} detail="Fecha de la obra" /></div></section>

    <PlanningCard title="Fases y tareas registradas" action={<Link href={`/obras/${work.id}/planificacion/gantt`}>Ver todas</Link>}><div className="planning-list">{buildHierarchy(tasks).filter((item) => item.depth === 0).slice(0, 6).map(({ task }) => <Link key={task.id} href={`/tareas/${task.id}`}><span className={`planning-task-dot planning-task-dot--${taskTone(task, new Date(work.nowIso))}`} /><strong>{task.title}</strong><span>{taskProgress(task) == null ? statusLabel(task.status) : `${taskProgress(task)}%`}</span></Link>)}</div></PlanningCard>
    <PlanningCard title="Hitos principales" action={<Link href={`/obras/${work.id}/planificacion/hitos`}>Ver hitos</Link>}><div className="planning-list">{upcoming.length ? upcoming.map((task) => <Link key={task.id} href={`/tareas/${task.id}`}><CalendarDays size={14} /><strong>{task.title}</strong><span>{formatCompactDate(task.dueAt)}</span></Link>) : <p className="planning-honest-empty">No hay tareas identificadas como hito con fecha registrada.</p>}</div></PlanningCard>
    ...
    <PlanningCard title="Entrega de materiales" action={<Link href={`/obras/${work.id}/planificacion/recursos`}>Ver materiales</Link>}><div className="planning-list planning-list--compact">{visibleMaterials.map((material) => <span key={material.id}><Package size={13} /><strong>{material.name}</strong><em>{statusLabel(material.status)}</em></span>)}{visibleMaterials.length === 0 ? <p className="planning-honest-empty">No hay materiales registrados.</p> : null}</div></PlanningCard>
    <PlanningCard title="Calendario de inspecciones" action={<Link href={`/obras/${work.id}/planificacion/calendario`}>Abrir calendario</Link>}><div className="planning-list planning-list--compact">{inspections.map((event) => <Link key={event.id} href="/agenda"><CheckCircle2 size={13} /><strong>{event.title}</strong><span>{formatCompactDate(event.startsAt)}</span></Link>)}{inspections.length === 0 ? <p className="planning-honest-empty">No hay inspecciones o controles registrados.</p> : null}</div></PlanningCard>
    <PlanningCard title="Permisos y decisiones" action={canManage ? <Link href={`/gestion?tipo=documento&clientId=${work.clientId}&workId=${work.id}`}>Registrar</Link> : undefined}><div className="planning-control-state"><CircleDot size={20} /><strong>Sin estado supuesto</strong><p>Permisos y decisiones aparecen sólo desde registros verificables.</p></div></PlanningCard>
  </div>;
}
...
export function WorkPlanningGantt({ work, tasks, canManage }: PlanningProps) {
  ...
  return <div className="planning-gantt-page">
    <section className="planning-gantt-toolbar" aria-label="Controles del diagrama de Gantt">
      <label><span className="sr-only">Filtrar por estado</span><Filter size={15} /><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Todas las tareas</option><option value="inbox">Entrada</option><option value="planned">Planificadas</option><option value="in_progress">En curso</option><option value="blocked">Bloqueadas</option><option value="completed">Completadas</option></select></label>
      ...
      <span className="planning-visible-count">{Math.min(30, visibleCount)} de {visibleCount} visibles</span>
