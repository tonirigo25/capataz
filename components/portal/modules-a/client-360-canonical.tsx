...
const SUMMARY_SHELL_VIEWS: Array<{ view: Client360CanonicalView; label: string }> = [
  { view: "resumen", label: "Resumen" },
  { view: "actividad", label: "Relación" },
  { view: "obras", label: "Operación" },
  { view: "facturas", label: "Dinero" },
  { view: "documentos", label: "Archivos" },
];
...
function Client360ReferenceSummary({
  ...
  moreActions?: ReactNode;
  nextAction?: Client360CanonicalProps["nextAction"];
  insights: Client360Insight[];
  incidents: Client360Incident[];
      ...
      >
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(16rem,0.8fr)_auto] xl:items-center">
          ...
          <nav className="grid min-w-48 gap-2" aria-label={`Acciones de ${displayName}`}>
            ...
            {hrefs.newOpportunity ? (
              <span className="client-360-canonical__primary-action relative block">
                <Link href={hrefs.newOpportunity} className="primary-button w-full"><Plus size={16} aria-hidden="true" /> {hrefs.newOpportunityLabel ?? "Nueva oportunidad"}</Link>
                {moreActions ? <span className="client-360-canonical__more-actions">{moreActions}</span> : null}
              </span>
            ) : moreActions ? <span className="client-360-canonical__more-actions client-360-canonical__more-actions--standalone">{moreActions}</span> : null}
          </nav>
        </div>
      </section>

      <nav className="client-360-shell-summary__tabs" aria-label="Secciones de Cliente 360">
        {SUMMARY_SHELL_VIEWS.map(({ view, label }) => {
          const Icon = viewIcons[view];
          return <Link key={view} href={`/clientes/${client.id}?vista=${view}`} aria-current={view === "resumen" ? "page" : undefined}><Icon size={16} aria-hidden="true" />{label}</Link>;
        })}
      </nav>
        ...
        <Panel title="Próxima acción" icon={CalendarCheck2}>
          {nextAction ? (
            <div className="flex flex-1 flex-col">
              ...
              {nextAction.description ? <p className="mt-1 text-sm text-content-secondary">{nextAction.description}</p> : null}
              <dl className="mt-4 grid gap-2 text-sm">
                {nextAction.dateLabel ? <Fact label="Fecha" value={nextAction.dateLabel} /> : null}
                {nextAction.contactLabel ? <Fact label="Con" value={nextAction.contactLabel} /> : null}
              </dl>
              {nextAction.completeHref ? <Link href={nextAction.completeHref} className="primary-button mt-auto w-full pt-3"><CheckCircle2 size={16} aria-hidden="true" />{nextAction.actionLabel ?? "Revisar y completar"}</Link> : null}
            </div>
  ...
  return (
    <section className={`min-w-0 rounded-xl border border-border bg-surface p-4 shadow-soft ${className}`}>
      <header className="mb-4 flex items-center justify-between gap-3">
        <h2 className="inline-flex items-center gap-2 font-semibold text-content"><Icon size={17} aria-hidden="true" />{title}</h2>
        {href ? <Link href={href} className="text-xs font-semibold text-brand-strong hover:underline">Ver detalle</Link> : null}
      </header>
      {children}
    </section>
  );
