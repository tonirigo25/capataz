...
export async function processStripeEvent(event: Stripe.Event) {
  ...
  if (existing && isTerminalBillingEventResult(existing.status)) {
    return { duplicate: true, result: existing.status };
  }
  const companyId = existing?.companyId ?? await companyIdForEvent(event);
  ...
  const record = existing
    ...
    : await prisma.billingEvent.create({
      data: {
        ...
        payload: eventProjection(event),
        payloadHash: createHash("sha256").update(JSON.stringify(eventProjection(event))).digest("hex"),
  ...
  try {
    const result = await applyEvent(event, record.occurredAt ?? eventCreatedAt);
    await prisma.billingEvent.update({
      where: { id: record.id },
      data: { companyId: result.companyId ?? companyId, processedAt: new Date(), status: result.result },
    });
    ...
    await prisma.billingEvent.update({
      where: { id: record.id },
      data: { processedAt: new Date(), status: `FAILED:${code}`, lastError: code },
    });
    throw error;
  }
}
...
async function companyIdForCustomer(customerId: string | null) {
  ...
  return (await prisma.billingCustomer.findUnique({
    where: { provider_externalCustomerId: { provider: "stripe", externalCustomerId: customerId } },
    select: { companyId: true },
  }))?.companyId ?? null;
}

async function companyIdForEvent(event: Stripe.Event) {
  if (event.type === "checkout.session.completed") {
    const session = event.data.object as Stripe.Checkout.Session;
    return session.metadata?.companyId || session.client_reference_id || companyIdForCustomer(idOf(session.customer));
  }
  if (event.type.startsWith("customer.subscription.")) {
  ...
  if (event.type === "invoice.paid" || event.type === "invoice.payment_failed") {
    const invoice = event.data.object as Stripe.Invoice;
    return companyIdForCustomer(idOf(invoice.customer));
  }
  return null;
}
