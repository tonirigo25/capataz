...
export async function globalSearch(query: string) {
  ...
  budgets.forEach((budget) => results.push({ type: "Presupuestos", title: `${budget.numero} · ${budget.titulo}`, detail: `${budget.client.nombre} · ${statusLabel(budget.estado)} · ${budget.total} €`, href: `/presupuestos/${budget.id}` }));
  invoices.forEach((invoice) => {
    const liveStatus = deriveInvoiceStatus(invoice.total, invoice.pendiente, invoice.fechaVencimiento);
    results.push({ type: "Facturas", title: `${invoice.numero} · ${invoice.client.nombre}`, detail: `${invoice.concepto} · ${statusLabel(liveStatus)} · pendiente ${invoice.pendiente} €`, href: `/dinero/${invoice.id}` });
  });
