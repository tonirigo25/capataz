...
export async function runProviderContractSuite(providers: { billing: BillingProvider; email: EmailDeliveryProvider; storage: StorageProvider; ai: AiGatewayProvider; fiscal: FiscalTransmissionProvider; observability: ObservabilityProvider }) {
  ...
  const deleted = await providers.storage.delete({ companyId: "contract-company", objectKey: "contract/object.bin", idempotencyKey: "storage-delete-contract" });
  ...
  for (const [name, value, key] of [["billing", billing, "billing-contract"], ["portal", portal, "portal-contract"], ["email", email, "email-contract"], ["storage", stored, "storage-contract"], ["storage-delete", deleted, "storage-delete-contract"], ["ai", ai, "ai-contract"], ["fiscal", fiscal, "fiscal-contract"]] as const) {
