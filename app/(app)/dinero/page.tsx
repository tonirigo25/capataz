...
export default async function MoneyPage({ searchParams }: { searchParams: Promise<{ filtro?: string; buscar?: string }> }) {
  ...
  const [workIds, clientIds] = await Promise.all([resolveScopedEntityIds(auth, "sales.invoices.view", "Work"), resolveScopedEntityIds(auth, "sales.invoices.view", "Client")]);
  const scopeWhere = relationScope(auth.scope, workIds, clientIds);
  const [createDecision, updateDecision, collectDecision, agendaDecision] = await Promise.all([
    resolveAuthorization(auth, "sales.invoices.create"), resolveAuthorization(auth, "sales.invoices.create"),
    resolveAuthorization(auth, "treasury.collections.register"), resolveAuthorization(auth, "agenda.manage")
  ]);
  const [updateWorkIds, updateClientIds, collectWorkIds, collectClientIds, agendaWorkIds, agendaClientIds] = await Promise.all([
    updateDecision.allowed ? resolveScopedEntityIds(auth, "sales.invoices.create", "Work") : Promise.resolve([]),
    updateDecision.allowed ? resolveScopedEntityIds(auth, "sales.invoices.create", "Client") : Promise.resolve([]),
    collectDecision.allowed ? resolveScopedEntityIds(auth, "treasury.collections.register", "Work") : Promise.resolve([]),
    collectDecision.allowed ? resolveScopedEntityIds(auth, "treasury.collections.register", "Client") : Promise.resolve([]),
    agendaDecision.allowed ? resolveScopedEntityIds(auth, "agenda.manage", "Work") : Promise.resolve([]),
    agendaDecision.allowed ? resolveScopedEntityIds(auth, "agenda.manage", "Client") : Promise.resolve([])
  ]);
  const invoices = await prisma.invoice.findMany({ where: { companyId, ...scopeWhere }, orderBy: { fechaVencimiento: "asc" }, include: { client: true, work: true, payments: true } });
  const invoicesWithStatus: InvoiceListItem[] = invoices.map((invoice) => ({ ...invoice, liveStatus: invoice.estado === "borrador" ? "borrador" : deriveInvoiceStatus(invoice.total, invoice.pendiente, invoice.fechaVencimiento), ui: {
    update: updateDecision.allowed && relationAllowed(updateDecision.scope, updateWorkIds, updateClientIds, invoice),
    collect: collectDecision.allowed && relationAllowed(collectDecision.scope, collectWorkIds, collectClientIds, invoice),
    agenda: agendaDecision.allowed && relationAllowed(agendaDecision.scope, agendaWorkIds, agendaClientIds, invoice)
  } }));
...
function relationAllowed(scope: string, workIds: string[] | null, clientIds: string[] | null, entity: { obraId: string | null; clienteId: string }) { if (scope === "COMPANY") return true; if (scope === "SELECTED_WORKS") return Boolean(entity.obraId && workIds?.includes(entity.obraId)); if (scope === "SELECTED_CLIENTS") return Boolean(clientIds?.includes(entity.clienteId)); return entity.obraId ? Boolean(workIds?.includes(entity.obraId)) : Boolean(clientIds?.includes(entity.clienteId)); }
