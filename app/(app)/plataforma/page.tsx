...
import { createSupportGrant, closeSupportGrant, toggleCompanySuspension, updateDemoRequest } from "./actions";
...
export default async function PlatformPage() {
  ...
  return <main className="screen">
    ...
    {actor.platformRole === "PLATFORM_OWNER" ? <section className="mt-6" aria-labelledby="demo-requests-title">
      ...
      <div className="mt-3 grid gap-3">
        {demoRequests.length ? demoRequests.map((request) => <article className="card p-4" key={request.id}>
          ...
          <form action={updateDemoRequest} className="mt-4 grid gap-2 sm:grid-cols-[minmax(10rem,.5fr)_minmax(14rem,1fr)_auto]">
            <input name="requestId" type="hidden" value={request.id}/>
            <label><span className="label">Estado y retención</span><select className="field mt-1" name="status" defaultValue={request.status}><option>PENDING</option><option>IN_REVIEW</option><option>QUALIFIED</option><option>DECLINED</option><option>CONVERTED</option><option>LEGAL_HOLD</option><option>SPAM</option></select></label>
            <label><span className="label">Motivo obligatorio</span><input className="field mt-1" name="reason" maxLength={240} required placeholder="Decisión, responsable o referencia"/></label>
            <button className="secondary-button self-end">Guardar gestión</button>
          </form>
