...
import { AlertTriangle, Archive, CheckCircle2, ClipboardSignature, Download, Eye, FileArchive, FileText, FolderOpen, Plus, Receipt, ScanLine, ScrollText, ShieldCheck } from "lucide-react";
...
export default async function DocumentsPage() {
  ...
  const canManageReceivedInvoices = (await resolveAuthorization(auth, "purchases.received_invoices.manage")).allowed;
  ...
  const [budgets, invoices, repositoryDocuments, inboxDocuments] = await Promise.all([
    ...
    <ListWorkspace>
      <SectionHeader
        ...
        action={
          <div className="flex flex-wrap gap-2">
            {canManageReceivedInvoices ? <Link href="/gastos-materiales/lector" className="primary-button">
              <Plus size={18} />
              Subir documento
            </Link> : null}
            {canCreateBudget ? <Link href="/gestion?tipo=presupuesto&returnTo=/documentos" className="secondary-button">
              <Plus size={18} />
              Presupuesto
            </Link> : null}
      ...
      <section className="mb-5">
        <div className="mb-3">
          ...
          <h2 className="type-title mt-1">Revisión con el original a la vista</h2>
          <p className="type-body mt-2 max-w-3xl">
            La extracción es una propuesta. Comprueba el archivo, corrige los datos y confirma antes de registrar un gasto.
          </p>
        </div>
        {activeInboxDocument ? (
          <div className="overflow-hidden rounded-[var(--radius-panel)] border border-[var(--color-border)] bg-white shadow-[var(--shadow-panel)] lg:grid lg:min-h-[34rem] lg:grid-cols-[17rem_minmax(0,1fr)_21rem]">
            <div className="border-b border-[var(--color-border)] lg:border-b-0 lg:border-r">
              <div className="flex items-center justify-between gap-3 border-b border-[var(--color-border)] px-4 py-3">
                ...
                <span className="type-meta">{inboxDocuments.length} recientes</span>
              </div>
              <div className="divide-y divide-[var(--color-border)]">
                {inboxDocuments.map((document, index) => (
                  <Link
                    ...
                    className={`block p-4 transition hover:bg-slate-50 ${index === 0 ? "bg-lime-50/70" : ""}`}
                  >
                    <div className="flex items-start gap-3">
                      <span className="rounded-lg bg-slate-100 p-2 text-slate-600"><FileText size={18} /></span>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-black text-obra-ink">{document.name}</p>
                        <p className="mt-1 truncate text-xs text-slate-500">
                          {document.businessPartner?.commercialName || document.extractedIssuer || "Proveedor pendiente"}
                        </p>
                        <div className="mt-2"><InboxStatus status={document.status} /></div>
                      </div>
                    </div>
                  </Link>
                ))}
              ...
              <div className="mx-auto flex min-h-[25rem] max-w-[30rem] flex-col bg-white p-6 shadow-sm sm:p-8">
                <div className="text-center">
                  ...
                  <p className="mt-3 break-words font-mono text-sm font-black uppercase text-obra-ink">
                    {activeInboxDocument.originalName || activeInboxDocument.name}
                  </p>
                </div>
                <div className="mt-8 space-y-4 border-y border-slate-300 py-6 font-mono text-sm">
                  <DocumentDatum label="Emisor" value={activeInboxDocument.extractedIssuer || "Pendiente de revisar"} />
                  <DocumentDatum label="Factura" value={activeInboxDocument.extractedInvoiceNo || "Pendiente"} />
                  <DocumentDatum label="Fecha" value={activeInboxDocument.extractedIssueDate ? formatDate(activeInboxDocument.extractedIssueDate) : "Pendiente"} />
                  <DocumentDatum label="Total" value={activeInboxDocument.extractedTotal != null ? formatCurrency(activeInboxDocument.extractedTotal) : "Pendiente"} strong />
                </div>
                <p className="mt-auto pt-8 text-center text-xs text-slate-500">
                  {activeInboxDocument.storageKey ? "La vista completa se abre por una ruta privada y autenticada." : "Registro sintético de Review sin binario adjunto."}
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {activeInboxDocument.extractionConfidence != null ? (
                  <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-black text-emerald-800">
                    {Math.round(activeInboxDocument.extractionConfidence * 100)} % confianza
                  </span>
                ) : null}
              </div>
              <div className="mt-4 grid gap-3">
                <ReviewDatum label="Proveedor" value={activeInboxDocument.businessPartner?.commercialName || activeInboxDocument.extractedIssuer || "Por identificar"} />
                <ReviewDatum label="Trabajo" value={activeInboxDocument.work?.titulo || "Por asignar o gasto general"} />
                <ReviewDatum label="Estado" value={inboxStatusLabel(activeInboxDocument.status)} />
                <ReviewDatum label="Resultado" value={activeInboxDocument.expense ? `Gasto único · ${activeInboxDocument.expense.concepto}` : "Aún no se ha creado ningún gasto"} />
              </div>
              <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                <p className="flex items-center gap-2 text-sm font-black text-obra-ink"><ShieldCheck size={18} />Comprobaciones</p>
                <ul className="mt-3 grid gap-2 text-xs leading-5 text-slate-600">
                  ...
                  <li>{activeInboxDocument.sha256 ? `✓ Huella SHA-256 · ${activeInboxDocument.sha256.slice(0, 12)}…` : "! Huella pendiente"}</li>
                  <li>{activeInboxDocument.status === "POSSIBLE_DUPLICATE" ? "! Posible duplicado: requiere confirmación." : "✓ Sin duplicado confirmado en este estado."}</li>
                </ul>
              </div>
              <Link href={`/gastos-materiales/lector/${activeInboxDocument.id}`} className="secondary-button mt-4 w-full justify-center">
                {activeInboxDocument.status === "REGISTERED" ? <CheckCircle2 size={18} /> : <AlertTriangle size={18} />}
                {activeInboxDocument.status === "REGISTERED" ? "Abrir registro" : "Revisar y confirmar"}
              </Link>
              {activeInboxDocument.storageKey ? <Link href={`/gastos-materiales/lector/${activeInboxDocument.id}/archivo`} target="_blank" className="mt-3 block text-center text-sm font-bold underline decoration-slate-300 underline-offset-4">
                Abrir original
              </Link> : null}
            </aside>
          </div>
          ...
          <div className="card p-6">
            ...
            <p className="mt-2 text-sm text-slate-600">Todavía no hay justificantes dentro de tu alcance. Al subir uno, aparecerán juntos el original, la propuesta y sus comprobaciones.</p>
            {canManageReceivedInvoices ? <Link href="/gastos-materiales/lector" className="secondary-button mt-4">Abrir subida segura</Link> : null}
          </div>
        )}
      </section>

      <div className="mb-3">
        <p className="type-eyebrow">Biblioteca</p>
        <h2 className="type-title mt-1">Documentos del negocio</h2>
      </div>
...
function InboxStatus({ status }: { status: string }) {
  const warning = ["REVIEW_REQUIRED", "AWAITING_PARTNER", "AWAITING_WORK", "POSSIBLE_DUPLICATE", "FAILED"].includes(status);
  const complete = ["READY", "REGISTERED", "SAVED"].includes(status);
  return <span className={`rounded-full px-2 py-1 text-[11px] font-black ${complete ? "bg-emerald-100 text-emerald-800" : warning ? "bg-orange-100 text-orange-800" : "bg-slate-100 text-slate-700"}`}>{inboxStatusLabel(status)}</span>;
}
...
function DocumentDatum({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return <div className="flex items-start justify-between gap-4"><span className="text-slate-500">{label}</span><span className={`text-right ${strong ? "font-black" : "font-bold"}`}>{value}</span></div>;
}

function ReviewDatum({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-slate-200 p-3"><p className="label">{label}</p><p className="mt-1 text-sm font-bold text-obra-ink">{value}</p></div>;
}
