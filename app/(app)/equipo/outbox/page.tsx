...
export default async function EmailOutboxPage() {
  ...
  return <main className="screen"><p className="type-label">Staging · correo local</p><h1 className="type-page-title mt-2">Bandeja interna</h1><p className="type-secondary mt-2">Vista protegida para validar destinatario, asunto y plantilla sin realizar entregas externas.</p><div className="mt-6 grid gap-3">{items.length ? items.map((item) => <article key={item.id} className="card p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><strong>{item.subject}</strong><p className="type-secondary">Para {item.recipient} · {item.status}</p><div className="prose mt-3 text-sm" dangerouslySetInnerHTML={{ __html: item.htmlBody ?? "" }}/></div>{["PENDING", "FAILED", "RETRYING"].includes(item.status) ? <LocalOutboxProcessor outboxId={item.id}/> : null}</div></article>) : <div className="rounded-xl border border-dashed border-border bg-subtle p-6"><h2 className="type-object-title text-content">No hay mensajes en la bandeja</h2><p className="type-secondary mt-2">Las invitaciones y avisos de acceso aparecerán aquí para su revisión local, sin envíos externos.</p></div>}</div></main>;
}
