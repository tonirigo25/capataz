...
export default async function ExpenseDocumentReaderPage({ searchParams }: { searchParams: Promise<{ error?: string; deleted?: string; estado?: string; buscar?: string }> }) {
  ...
  const documents = await prisma.document.findMany({
    where: {
      companyId, ...purchaseDocumentWhere(access.read), metadata: { path: ["source"], equals: "expense_document_reader" }, archivedAt: null,
      ...(status ? { status } : {}),
      ...(query.buscar ? { OR: [
  ...
  return <main className="screen">
    ...
    {access.manage.allowed && access.manage.scope === "COMPANY" ? <section className="card mb-6 p-4 sm:p-6">
    ...
    <section><div className="mb-3"><h2 className="text-lg font-black text-obra-ink">Bandeja de documentos</h2><p className="text-sm text-slate-600">Busca por estado, proveedor, trabajo, factura o archivo.</p><CompactFilterBar className="mt-3"><form className="grid gap-2 sm:grid-cols-[minmax(12rem,1fr)_13rem_auto]"><label><span className="label mb-1 block">Buscar</span><CompactSearch name="buscar" defaultValue={query.buscar || ""} placeholder="Proveedor o factura…" /></label><label><span className="label mb-1 block">Estado</span><select className="field" name="estado" defaultValue={query.estado || ""}><option value="">Todos</option>{statusOptions.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></label><button className="primary-button self-end" type="submit"><Search size={17} />Aplicar</button></form></CompactFilterBar></div><ResultCount shown={documents.length} total={documents.length} noun="documentos" /><div className="mt-3 grid gap-3 lg:grid-cols-2">
