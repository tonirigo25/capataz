...
export default async function SupplierInvoicePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    <BreadcrumbPrelude items={[{ label: "Dinero", href: "/dinero" }, { label: "Facturas de proveedor", href: "/facturas-proveedor" }, { label: "Detalle" }]} />
    <PurchaseInvoiceProfile companyId={auth.companyId} kind="SUPPLIER" id={id} searchParams={searchParams} access={await getPurchaseAccess(auth)} />
  </>;
}
