...
export default async function OnboardingPage() {
  ...
  return <main className="screen max-w-3xl">
    <header className="mb-6">
      <p className="type-label">Configura tu espacio · paso {state.step ?? 1} de 2</p>
      <h1 className="type-page-title mt-2">Adapta {brand.productName} a tu negocio</h1>
      <p className="type-secondary mt-2">Puedes guardar, volver atrás y continuar en otro momento. No se crearán datos de ejemplo.</p>
    </header>
    <form action={saveBusinessOnboarding} className="grid gap-5">
      <section className="card p-4">
        <h2 className="type-section-title">1. Lo imprescindible</h2>
      ...
      <details className="card p-4" open={state.step === 2}>
        <summary className="type-section-title cursor-pointer">2. Personalización opcional</summary>
        <p className="type-secondary mt-2">Estas respuestas afinan el lenguaje y el siguiente paso; puedes omitirlas sin perder acceso.</p>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:justify-between">
        <Link className="ghost-button" href="/hoy">Volver a Hoy</Link>
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <button className="primary-button" name="complete" value="true">Terminar configuración</button>
        </div>
      </div>
    </form>
  </main>;
