...
Decisión aplicada: `NEXT_PUBLIC_APP_ENV` quedó en `production` para aplicación y evaluador, mientras `NEXT_PUBLIC_APP_MODE=test` se conserva en la aplicación hasta disponer de suscripción y límites comerciales reales. Tras el redespliegue, healthcheck confirmó `appEnv=production`, base operativa y voz configurada; aplicación y cron quedaron en `SUCCESS` sobre `4d98202`.

El smoke productivo utilizó exclusivamente entidades internas con prefijo QA: checklist, completar/reabrir, subtarea, desambiguación entre dos dependencias, alta y retirada de relación, intento y reprogramación de FollowUp, creación/modificación/dry run/archivado de borrador y archivado confirmado de Task. No hubo mutaciones financieras ni comunicaciones externas. Tareas, seguimientos, automatización y conversaciones QA quedaron archivados. Las rutas operativas devolvieron 200, no aparecieron 5xx y `/capataz` pasó 390, 768 y 1440 px sin overflow ni errores de consola.
