...
if(mode==="integrations"){const today=read("app/(app)/hoy/page.tsx");["buildPortalManifest","agendaVisible ? getAgendaItems()","homeWidgets.slice(0, 3)","portal.quickActions"].forEach(item=>assert.ok(today.includes(item),`app/(app)/hoy/page.tsx: falta ${item}`));assert.doesNotMatch(today,/buildTodayDashboard|getEconomicControl|getTodayOperationalSignals/,"Hoy no debe recuperar datos company-wide fuera del PortalManifest");for(const p of ["clientes","obras","dinero","presupuestos"])has(`app/(app)/${p}/[id]/page.tsx`,["EntityWorkflowSummary"])}
