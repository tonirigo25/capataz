...
assert.equal(existsSync(join(root, "app/.well-known/security.txt/route.ts")), false);
assert.equal(existsSync(join(root, "public/.well-known/security.txt")), false);
assert.ok(read("docs/runbooks/RESPONSIBLE_DISCLOSURE.md").includes("READY_FOR_EXTERNAL_INPUT"));
assert.ok(read("app/seguridad/page.tsx").includes("No se publica todavía un canal responsable"));
...
process.stdout.write(`${JSON.stringify({ ok: true, controls: ["E", "F", "G"], securityTxt: "READY_FOR_EXTERNAL_INPUT", providerRunbooks: 6, debtItems: 6 })}\n`);
