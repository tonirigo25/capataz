...
const databaseName = "capataz_test_readiness_f3";
let runtime;
const removeSignalHandlers = installCleanupSignalHandlers(async () => stopIsolatedPostgres(runtime).catch(() => undefined));

try {
  runtime = await startIsolatedPostgres({ EmbeddedPostgres, root, suite: "readiness-f3-fiscal", password, postgresFlags: ["-c", "io_method=sync"] });
  ...
  execFileSync(process.execPath, [join(process.cwd(), "node_modules", "tsx", "dist", "cli.mjs"), "scripts/readiness/validate-f3-postgres.ts"], { cwd: process.cwd(), env, stdio: "inherit" });
} finally {
