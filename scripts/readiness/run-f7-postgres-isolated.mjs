...
const databaseName = "capataz_test_readiness_f7";
let runtime;
const removeSignalHandlers = installCleanupSignalHandlers(async () => stopIsolatedPostgres(runtime).catch(() => undefined));
try {
  runtime = await startIsolatedPostgres({ EmbeddedPostgres, root, suite: "readiness-f7-product", password, postgresFlags: ["-c", "io_method=sync"] });
  await runtime.pg.createDatabase(databaseName);
  const env = { ...process.env, DATABASE_URL: `postgresql://postgres:${password}@127.0.0.1:${runtime.port}/${databaseName}?schema=public`, CAPATAZ_TEST_DATABASE_ISOLATED: "true", APP_ENV: "test", NEXT_PUBLIC_APP_ENV: "test", NODE_ENV: "test", AI_ENABLED: "false", AI_PROVIDER_MODE: "off", BILLING_ENABLED: "false", EMAIL_ENABLED: "false", PRIVATE_STORAGE_ROOT: join(runtime.dataDir, "f7-private-storage") };
  ...
  execFileSync(process.execPath, [join(process.cwd(), "node_modules", "tsx", "dist", "cli.mjs"), "scripts/readiness/validate-f7-postgres.ts"], { cwd: process.cwd(), env, stdio: "inherit" });
} finally {
