...
const databaseName = "capataz_test_external_c2";
let runtime;
const removeSignalHandlers = installCleanupSignalHandlers(async () =>
  stopIsolatedPostgres(runtime).catch(() => undefined),
);
...
try {
  ...
  const env = {
    ...
    EMAIL_TOKEN_DERIVATION_SECRET: "c2-email-token-derivation-secret-32-bytes",
  };
  ...
  execFileSync(
    process.execPath,
    [join(process.cwd(), "node_modules", "tsx", "dist", "cli.mjs"), "scripts/readiness/validate-c2-postgres.ts"],
    { cwd: process.cwd(), env, stdio: "inherit" },
  );
} finally {
