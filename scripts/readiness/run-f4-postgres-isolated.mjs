...
const databaseName = "capataz_test_readiness_f4";
let runtime;
const removeSignalHandlers = installCleanupSignalHandlers(async () => stopIsolatedPostgres(runtime).catch(() => undefined));
try {
  runtime = await startIsolatedPostgres({ EmbeddedPostgres, root, suite: "readiness-f4-commercial", password, postgresFlags: ["-c", "io_method=sync"] });
  await runtime.pg.createDatabase(databaseName);
  const env = { ...process.env, DATABASE_URL: `postgresql://postgres:${password}@127.0.0.1:${runtime.port}/${databaseName}?schema=public`, CAPATAZ_TEST_DATABASE_ISOLATED: "true", APP_ENV: "test", NEXT_PUBLIC_APP_ENV: "test", NODE_ENV: "test", EMAIL_PROVIDER: "local", APP_BASE_URL: "http://localhost:3000", EMAIL_TOKEN_DERIVATION_SECRET: "f4-email-token-derivation-secret-32-bytes", STORAGE_SIGNING_SECRET: "f4-storage-signing-secret-32-bytes!!" };
  ...
  execFileSync(process.execPath, [join(process.cwd(), "node_modules", "tsx", "dist", "cli.mjs"), "scripts/readiness/validate-f4-postgres.ts"], { cwd: process.cwd(), env, stdio: "inherit" });
} finally {
