...
const databaseName = "capataz_test_readiness_f5";
let runtime;
const removeSignalHandlers = installCleanupSignalHandlers(async () => stopIsolatedPostgres(runtime).catch(() => undefined));
try {
  runtime = await startIsolatedPostgres({ EmbeddedPostgres, root, suite: "readiness-f5-privacy-security", password, postgresFlags: ["-c", "io_method=sync"] });
  await runtime.pg.createDatabase(databaseName);
  const env = { ...process.env, DATABASE_URL: `postgresql://postgres:${password}@127.0.0.1:${runtime.port}/${databaseName}?schema=public`, CAPATAZ_TEST_DATABASE_ISOLATED: "true", APP_ENV: "test", NEXT_PUBLIC_APP_ENV: "test", NODE_ENV: "test", STORAGE_SIGNING_SECRET: "f5-storage-signing-secret-32-bytes!!", JOB_RUNNER_SECRET: "f5-isolated-job-secret" };
  ...
  execFileSync(process.execPath, [join(process.cwd(), "node_modules", "tsx", "dist", "cli.mjs"), "scripts/readiness/validate-f5-postgres.ts"], { cwd: process.cwd(), env, stdio: "inherit" });
} finally {
