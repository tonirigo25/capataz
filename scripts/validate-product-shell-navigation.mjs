...
check("sidebar abre Más áreas con un activador real", chrome.includes('openDesktopPanel("more", trigger)') && chrome.includes('aria-controls="desktop-more-navigation"') && chrome.includes('id="desktop-more-navigation"'));
check("sidebar despliega submódulos estables bajo su módulo principal", navigation.includes("export const productSubnavigation") && ["/oportunidades", "/seguimientos", "/tareas", "/tesoreria", "/recordatorios", "/equipos", "/automatizaciones"].every((route) => navigation.includes(`href: "${route}"`)) && chrome.includes("<NavigationBranch") && chrome.includes("Submenús de ${item.label}"));
check("submenús respetan capacidades y sólo se expanden en el contexto activo", chrome.includes("capabilitySet.has(item.capability)") && chrome.includes("parentActive || childActive") && chrome.includes("children.length > 0") && chrome.includes('data-expanded={expanded ? "true" : "false"}'));
check("Actividad estricta no se ofrece como submenú ni enlace sin capacidades y scope equivalentes", !navigation.slice(navigation.indexOf('"/obras": ['), navigation.indexOf('"/dinero": [')).includes('href: "/actividad"') && ["reports.view", "clients.view", "work.view", "sales.budgets.view", "sales.invoices.view", "treasury.view", "purchase_cost.view", "agenda.view", "documents.view"].every((capability) => chrome.includes(`"${capability}"`)) && chrome.includes('scope.scope !== "COMPANY"') && chrome.includes('item.href !== "/actividad" || activityRouteAvailable'));
check("Más escritorio elimina sólo destinos realmente visibles en sidebar sin ocultar Compras", chrome.includes("sidebarNavigation.flatMap") && chrome.includes("item.children ?? []") && navigation.includes('label: "Compras"'));
...
check("Más recibe foco al abrir sin recorrer la página", chrome.includes('desktopPanel === "more"') && chrome.includes("panelRef.current?.focus()") && chrome.includes('tabIndex={-1}') && chrome.includes('aria-label="Más áreas"'));
