...
test("Dinero exige alcance COMPANY en todas las capacidades antes de cargar agregados", () => { const page = readFileSync(join(root, "app/(app)/dinero/page.tsx"), "utf8"); assert.match(page, /auth\.scope === "COMPANY"/); assert.match(page, /decision\.allowed && decision\.scope === "COMPANY"/); assert.match(page, /surface="money"/); });
test("Tesorería deja de ser un destino de navegación pero conserva sus detalles enlazados", () => { const navigation = readFileSync(join(root, "lib/product-navigation.ts"), "utf8"); const center = readFileSync(join(root, "components/economic-control-center.tsx"), "utf8"); assert.doesNotMatch(navigation, /href: "\/tesoreria"/); assert.match(center, /action="\/tesoreria"|returnTo=\/tesoreria|economicHref/); });
