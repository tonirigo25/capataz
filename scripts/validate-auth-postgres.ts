...
async function main() {
  ...
  try {
    runtime = await startIsolatedPostgres({
      ...
      suite: "auth-postgres",
      password,
      preferredPort: process.env.CAPATAZ_AUTH_POSTGRES_PORT,
    });
    await runtime.pg.createDatabase("capataz_test_auth");
    const env = { ...process.env, DATABASE_URL: `postgresql://postgres:${password}@127.0.0.1:${runtime.port}/capataz_test_auth?schema=public`, CAPATAZ_TEST_DATABASE_ISOLATED: "true", APP_ENV: "test", NEXT_PUBLIC_APP_ENV: "test" };
