...
check("Hoy no consulta módulos ausentes del portal", !today.includes("getDashboardData") && !today.includes("getEconomicControl") && !today.includes("getTodayOperationalSignals") && today.includes("agendaVisible ? getAgendaItems()"));
check("Hoy retira la rejilla heredada de seis KPI", !today.includes("StatCard") && !today.includes("Estado del negocio"));
check("Hoy deriva captura rápida del PortalManifest", today.includes("portal.quickActions.map") && today.includes("Captura rápida según permisos"));
check("Hoy incluye agenda inmediata solo si está autorizada", today.includes("agendaVisible ?") && today.includes("Agenda de hoy"));
check("Hoy presenta un máximo de tres prioridades distintas por perfil", today.includes("portal.homeWidgets") && today.includes(".slice(0, 3)") && today.includes("homeWidgetReason") && today.includes("homeWidgetImpact"));
check("Hoy explica motivo, origen, impacto y acción", ["reason:", "origin:", "impact:", "action:", "Origen:", "Impacto:"].every((token) => today.includes(token)));
check("Hoy elimina actividad company-wide", !today.includes("recentActivity") && !today.includes("Actividad reciente"));
