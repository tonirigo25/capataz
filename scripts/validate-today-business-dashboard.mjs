...
check("Hoy deriva acciones rápidas del PortalManifest", today.includes("portal.quickActions.map") && today.includes("Acciones rápidas"));
check("Hoy incluye agenda inmediata solo si está autorizada", today.includes("agendaVisible ?") && today.includes("Agenda de hoy"));
check("Hoy presenta prioridades distintas por perfil", /portal\.homeWidgets(?:\.slice\([^)]*\))?\.map/.test(today) && today.includes("homeWidgetDescription"));
